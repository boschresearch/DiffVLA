        ...
        **kwargs,
        ...
        for b in range(batch_size):
            ...
            for c in range(on_img.shape[0]):
                ...
                # import cv2
                # import numpy as np
                # depth_save = (depth[b, c].squeeze().detach().cpu().numpy() * 1000).astype(np.uint16)
                # cv2.imwrite("debug_vis/img_depth.png", depth_save)
...
class LSSTransformBEVDepth(nn.Module):
    ...
    def get_cam_feats(self, x):
        ...
        x = self.depthnet(x)
        depth = x[:, : self.D].softmax(dim=1)  # (B, N, self.D, H, W)
        x = depth.unsqueeze(1) * x[:, self.D : (self.D + self.C)].unsqueeze(2)
    ...
    def get_downsampled_gt_depth(self, gt_depths):
        ...
        gt_depths = gt_depths.view(
            B * N,
            H // self.img_downsample,
            self.img_downsample,
            W // self.img_downsample,
            self.img_downsample,
            1,
        )
        gt_depths = gt_depths.permute(0, 1, 3, 5, 2, 4).contiguous()
        gt_depths = gt_depths.view(-1, self.img_downsample * self.img_downsample)
        gt_depths_tmp = torch.where(
            gt_depths == 0.0, 1e5 * torch.ones_like(gt_depths), gt_depths
        )
        gt_depths = torch.min(gt_depths_tmp, dim=-1).values
        gt_depths = gt_depths.view(B * N, H // self.img_downsample, W // self.img_downsample)
        ...
        gt_depths = (
            ...
            * (self.D - 1)
            / torch.log(
                torch.tensor(self.dbound[1] - 1.0).float()
                / self.dbound[0]
            )
        )
        gt_depths = gt_depths + 1.0

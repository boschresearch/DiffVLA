...
student_ori_augmentation = transforms.Compose(
            [
                transforms.ToTensor(),
                color_jittering,
                GaussianBlur(p=0.1),
...
class ImageAug3D(BaseTransform):
    ...
    def sample_augmentation(self, results):
        H, W = results["metas"]["ori_shapes"][0]
    ...
    def img_transform(
        ...
        # add color and gaussian aug
        # img = student_ori_augmentation(img)
    ...
    def transform(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        data["metas"]["img_aug_matrix"] = transforms
        return data
...
class ImageNorm:
    ...
    def __call__(self, data: Dict[str, Any]):
        imgs = data["imgs"]  # [(H, W, 3)] * num_cams
        new_imgs = []  # [(3, H, W)] * num_cams
        for img in imgs:
            new_img = tf.ToTensor()(img.astype(np.uint8))
            new_imgs.append(new_img)

        data["imgs"] = torch.stack(new_imgs, axis=0)  # (num_cams, 3, H, W)
        return data

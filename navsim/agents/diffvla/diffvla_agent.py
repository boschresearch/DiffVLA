...
from navsim.agents.diffvla.diffvla_model import DiffvlaModelV2

from navsim.agents.diffvla.diffvla_callback import DiffvlaCallback
from navsim.agents.diffvla.diffvla_loss import diffvla_loss
from navsim.agents.diffvla.diffvla_features import DiffvlaFeatureBuilder, DiffvlaTargetBuilder
from navsim.common.dataclasses import SensorConfig
from navsim.planning.training.abstract_feature_target_builder import (
    AbstractFeatureBuilder,
    AbstractTargetBuilder,
)
from navsim.agents.diffvla.modules.scheduler import WarmupCosLR
...
class DiffvlaAgent(AbstractAgent):
    ...
    def init_from_pretrained(self):
        if self._checkpoint_path:
            if torch.cuda.is_available():
                ...
                self._diffvla_model.to(torch.device("cuda"))
            else:
    ...
    def get_feature_builders(self) -> List[AbstractFeatureBuilder]:
        """Inherited, see superclass."""
        return [DiffvlaFeatureBuilder(config=self._config, is_train=True)]

    def forward(
        self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor] = None
    ) -> Dict[str, torch.Tensor]:
        """Inherited, see superclass."""
        return self._diffvla_model(features, targets=targets)
    ...
    def get_step_lr_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=self._config.lr_steps, gamma=0.1
        )
    ...
    def get_feature_builders_test(self) -> List[AbstractFeatureBuilder]:
        """Inherited, see superclass."""
        return [DiffvlaFeatureBuilder(config=self._config, is_train=False)]
    ...
    def compute_trajectory(self, agent_input: AgentInput) -> Trajectory:
        ...
        features: Dict[str, torch.Tensor] = {}
        # build features
        for builder in self.get_feature_builders_test():
            features.update(builder.compute_features(agent_input))

        # add batch dimension
        device = torch.device("cuda")
        for k, v in features.items():
            if k in ["imgs", "status_feature"]:
    ...
    def compute_results(self, agent_input: AgentInput, scene) -> Trajectory:
        self.eval()
        features: Dict[str, torch.Tensor] = {}
        # build features
        for builder in self.get_feature_builders_test():
        ...
        targets: Dict[str, torch.Tensor] = {}
        for builder in self.get_target_builders_test():
        ...
        trajectory = Trajectory(poses, self._trajectory_sampling)

        return features, targets, predictions, trajectory

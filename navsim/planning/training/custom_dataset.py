...
def load_feature_from_npz(path: Path) -> Dict[str, torch.Tensor]:
    with open(path, "rb") as f:
        data = np.load(f, allow_pickle=True)
        array_data = list(data.values())[0].astype(np.float32)
    return {"dac_score_feature": torch.from_numpy(array_data)}
...
class BEVCacheOnlyDataset(torch.utils.data.Dataset):
    """Dataset wrapper for feature/target datasets from cache only."""
    ...
    def collate_batch(batch_list):
        # batch[0][0] 'imgs', 'points', 'status_feature'
        # batch[0][1] 'trajectory', 'agent_states', 'agent_labels', 'bev_semantic_map'
        import torch

        # features
        # features = dict({"imgs": [], "points": [], "status_feature": []})
        features = dict()

        # targets
        # targets = dict({"trajectory": [], "agent_states": [], "agent_labels": [], "bev_semantic_map": []})
        targets = dict()

        for i, cur_sample in enumerate(batch_list):
            # features: 'imgs', 'points', 'status_feature'
            for key, val in cur_sample[0].items():
                if key not in features.keys():
                    features[key] = []
                features[key].append(val)

            # targets: 'trajectory', 'agent_states', 'agent_labels', 'bev_semantic_map'
        ...
        for key, val in features.items():
            if key == "points" or key == "metas":
                continue
            else:
                features[key] = torch.stack(val, dim=0)  # (B, ...)

        for key, val in targets.items():
            targets[key] = torch.stack(val, dim=0)
...
class BEVTemporalCacheOnlyDataset(torch.utils.data.Dataset):
    """Dataset wrapper for feature/target datasets from cache only."""
...
class BEVTemporalDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        scene_loader: SceneLoader,
        feature_builders: List[AbstractFeatureBuilder],
